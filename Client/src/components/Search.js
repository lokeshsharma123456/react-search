...
function App() {
  ...
  const [researchArea, _setResearchArea] = React.useState(["PHYSICS", "MACHINE LEARNING", "ARTIFICIAL INTELLIGENCE", "CHEMISTRY"]);
  ...
  const fetchData = () => {
    const query = `${value.label} ${researchAreaSelection}`;
    ...
    fetch(`http://localhost:3001/search?p=${query}`)
    .then(res => res.json())
    .then(data => {
